    ...
    async function loadData() {
      ...
      try {
        ...
        const { data: quizzes } = await supabase
          .from('quizzes')
          .select('id, topic, difficulty, score, total_questions, time_taken, created_at')
          .eq('user_id', user.id)
              ...
              <tbody>
                {history.map((q) => (
                  <tr key={q.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-sm text-gray-800">{q.topic}</td>
                    <td className="px-4 py-2 text-sm capitalize">{q.difficulty}</td>
                    <td className="px-4 py-2 text-sm text-gray-800">
                      {q.score}/{q.total_questions}
                    </td>
                    <td className="px-4 py-2 text-sm text-gray-600">
                      {q.time_taken !== null
                        ? `${Math.floor(q.time_taken / 60)}:${String(q.time_taken % 60).padStart(2, '0')}m`
                        : '-'}
                    </td>
                  </tr>
                ))}
